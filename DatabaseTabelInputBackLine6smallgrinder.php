...
if(isset($_POST['submit'])){
	...
	$insert = mysql_query("UPDATE form_checklist_line_6_smallgrinder SET
				...
				Motor_Grinder_Ket = '$ket[$j]' WHERE Tanggal = '$date';");   $j++; $insert = mysql_query("UPDATE form_checklist_line_6_smallgrinder SET
				...
				Conveying_Fan1_Ket = '$ket[$j]' WHERE Tanggal = '$date';");   $j++; $insert = mysql_query("UPDATE form_checklist_line_6_smallgrinder SET
	...
	if(!$insert){
		echo "<h1><script>alert('Form Checklist Line 6 Small Grinder Tanggal ($date) Gagal Disimpan');</script></h1>";

echo "<h2><script>alert('Tidak Dapat Input Data Pada Tanggal Yang Sama');
		location.href=MainPageTab.php';</script></h2>";
	}
	else {
		include "WebTabelbacktobiggrinderline6.php";
	}						
}	
else{
	echo "Proses Salah";

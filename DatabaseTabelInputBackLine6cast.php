...
if(isset($_POST['submit'])){
	...
	$insert = mysql_query("UPDATE form_checklist_line_6_casting SET 	
	...
	$insert = mysql_query("UPDATE form_checklist_line_6_casting SET 
				...
				WHERE Tanggal = '$date';");
	if(!$insert){
		echo "<h1><script>alert('Form Checklist Line 6 Casting Tanggal ($date) Gagal Disimpan');</script></h1>";

echo "<h2><script>alert('Tidak Dapat Input Data Pada Tanggal Yang Sama');
		location.href='MainPageTab.php';</script></h2>";
	}
	else {
		include "WebTabelbacktomdoline6.php";
	}						
}	
else{
	echo "Proses Salah";
